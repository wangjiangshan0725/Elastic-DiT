...
class EDiTQwenImagePipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16):
        ...
        if False:
            self.model_fn = model_fn_qwen_image_adaptivelinear
        else:
    ...
    def from_pretrained(
        ...
        # Initialize pipeline
        pipe = EDiTQwenImagePipeline(device=device, torch_dtype=torch_dtype)
    ...
    def __call__(
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            noise_pred, probs_list = self.cfg_guided_model_fn(
                    self.model_fn, cfg_scale,
                    inputs_shared, inputs_posi, inputs_nega,
                    **models, timestep=timestep, progress_id=progress_id
                )
            inputs_shared["latents"] = self.step(self.scheduler, progress_id=progress_id, noise_pred=noise_pred, **inputs_shared)
            probs_list_total.append(probs_list)
        # Decode
...
class QwenImageUnit_ShapeChecker(PipelineUnit):
    ...
    def process(self, pipe: EDiTQwenImagePipeline, height, width):
...
class QwenImageUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: EDiTQwenImagePipeline, height, width, seed, rand_device, layer_num):
        if layer_num is None:
...
class QwenImageUnit_InputImageEmbedder(PipelineUnit):
    ...
    def process(self, pipe: EDiTQwenImagePipeline, input_image, noise, tiled, tile_size, tile_stride):
...
class QwenImageUnit_LayerInputImageEmbedder(PipelineUnit):
    ...
    def process(self, pipe: EDiTQwenImagePipeline, layer_input_image, tiled, tile_size, tile_stride):
...
class QwenImageUnit_Inpaint(PipelineUnit):
    ...
    def process(self, pipe: EDiTQwenImagePipeline, inpaint_mask, height, width, inpaint_blur_size, inpaint_blur_sigma):
...
class QwenImageUnit_PromptEmbedder(PipelineUnit):
    ...
    def encode_prompt(self, pipe: EDiTQwenImagePipeline, prompt):
    ...
    def encode_prompt_edit(self, pipe: EDiTQwenImagePipeline, prompt, edit_image):
    ...
    def encode_prompt_edit_multi(self, pipe: EDiTQwenImagePipeline, prompt, edit_image):
        template =  "<|im_start|>system\nDescribe the key features of the input image (color, shape, size, texture, objects, background), then explain how the user's text instruction should alter or modify the image. Generate a new image that meets the user's requirements while maintaining consistency with the original input where appropriate.<|im_end|>\n<|im_start|>user\n{}<|im_end|>\n<|im_start|>assistant\n"
        drop_idx = 64
    ...
    def process(self, pipe: EDiTQwenImagePipeline, prompt, edit_image=None) -> dict:
...
class QwenImageUnit_EntityControl(PipelineUnit):
    ...
    def get_prompt_emb(self, pipe: EDiTQwenImagePipeline, prompt) -> dict:
        if pipe.text_encoder is not None:
            prompt = [prompt]
            template = "<|im_start|>system\nDescribe the image by detailing the color, shape, size, texture, quantity, text, spatial relationships of the objects and background:<|im_end|>\n<|im_start|>user\n{}<|im_end|>\n<|im_start|>assistant\n"
            drop_idx = 34
    ...
    def process(self, pipe: EDiTQwenImagePipeline, inputs_shared, inputs_posi, inputs_nega):
...
class QwenImageUnit_BlockwiseControlNet(PipelineUnit):
    ...
    def process(self, pipe: EDiTQwenImagePipeline, blockwise_controlnet_inputs: list[ControlNetInput], tiled, tile_size, tile_stride):
...
class QwenImageUnit_EditImageEmbedder(PipelineUnit):
    ...
    def process(self, pipe: EDiTQwenImagePipeline, edit_image, tiled, tile_size, tile_stride, edit_image_auto_resize=False):
        if edit_image is None:
            return {}
        pipe.load_models_to_device(self.onload_model_names)
...
class QwenImageUnit_Image2LoRAEncode(PipelineUnit):
    ...
    def encode_images_using_siglip2(self, pipe: EDiTQwenImagePipeline, images: list[Image.Image]):
    ...
    def encode_images_using_dinov3(self, pipe: EDiTQwenImagePipeline, images: list[Image.Image]):
    ...
    def encode_images_using_qwenvl(self, pipe: EDiTQwenImagePipeline, images: list[Image.Image], highres=False):
    ...
    def encode_images(self, pipe: EDiTQwenImagePipeline, images: list[Image.Image]):
        if images is None:
            return {}
    ...
    def process(self, pipe: EDiTQwenImagePipeline, image2lora_images):
...
class QwenImageUnit_Image2LoRADecode(PipelineUnit):
    ...
    def process(self, pipe: EDiTQwenImagePipeline, image2lora_x, image2lora_residual, image2lora_residual_highres):
...
class QwenImageUnit_ContextImageEmbedder(PipelineUnit):
    ...
    def process(self, pipe: EDiTQwenImagePipeline, context_image, height, width, tiled, tile_size, tile_stride):
...
def model_fn_qwen_image(
    ...
    **kwargs
    ...
    gate_mlp_list = []
    # p_hard_accu=None
    for block_id, (block, gate_mlp) in enumerate(zip(dit.transformer_blocks, dit.gate_mlps)):
        # if block_id in [30,35,40,45,50]:
        #     continue
        # if block_id in [10,15,20,25,30]:
        #     continue
        text, image, prob, prob_mlp = gradient_checkpoint_forward(
            block,
        ...
        #     hard_idx = prob_mlp.argmax(dim=-1)                # [B]
        #     p_hard = F.one_hot(hard_idx, num_classes=4).to(
        #         dtype=prob_mlp.dtype, device=prob_mlp.device
        #     ) 
        #     p_hard_accu = p_hard_accu + p_hard if p_hard_accu is not None else p_hard


        probs_list.append(prob)
        gate_mlp_list.append(prob_mlp)
        if blockwise_controlnet_conditioning is not None:
    ...
    return latents, [probs_list, gate_mlp_list]
